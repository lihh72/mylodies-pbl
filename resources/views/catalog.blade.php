...
<body class="bg-[#fef8f2] text-[#5a4a3b] font-sans">
    ...
    <section id="rentals" class="bg-gradient-to-b from-[#fff8f2] to-[#f9f3ea] py-28 relative overflow-hidden">
        ...
        <!-- Decorative shapes -->
        <div
            class="absolute -top-10 -left-10 w-64 h-64 bg-[#f0e1ce] opacity-30 rounded-full blur-3xl pointer-events-none animate-pulse">
        </div>
        <div
            class="absolute bottom-0 right-0 w-40 h-40 bg-[#e9d4bd] opacity-20 rounded-full blur-2xl pointer-events-none">
        </div>

        <div class="max-w-8xl mx-auto px-6 relative z-10">
            <!-- Filters -->
            <div class="flex flex-wrap justify-center gap-4 mb-14">
                @foreach ([['icon' => 'keyboard', 'label' => 'Keyboards'], ['icon' => 'guitar', 'label' => 'Guitars'], ['icon' => 'music', 'label' => 'Winds'], ['icon' => 'drum', 'label' => 'Drums'], ['icon' => 'heart', 'label' => 'Favorites']] as $filter)
                    <button
                        class="flex items-center gap-3 px-5 py-3 rounded-full border border-[#d6b896] bg-gradient-to-r from-[#f6e8d6] to-[#d6b896] text-[#5a4a3b] text-sm font-medium transition-all transform hover:scale-105 shadow-lg hover:shadow-2xl hover:bg-[#f3d0a9] focus:outline-none focus:ring-2 focus:ring-[#b49875] focus:ring-opacity-50">
