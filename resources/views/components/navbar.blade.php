<header x-data="{ open: false }"
    class="fixed top-0 w-full z-50 bg-gradient-to-r from-[#b49875]/90 to-[#9c7b59]/90 shadow-md sm:backdrop-blur-md transition-transform duration-700 ease-in-out animate-fade-in-down"
    ...
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 py-4 flex justify-between items-center relative">
        ...
        <div class="flex items-center space-x-6">
            <a href="{{ route('landing') }}" class="flex items-center space-x-3 shrink-0">
                <img src="{{ asset('images/logo-background.png') }}" alt="MyLodies Logo"
                    class="h-9 drop-shadow-lg sm:hover:scale-110 transition duration-300 motion-safe:hover:scale-105"
                    loading="lazy">
                <span
                    class="hidden lg:flex text-xl font-bold text-white tracking-wide hover:text-[#f9e5c9] transition">
                    MyLodies
                </span>
            </a>
            ...
            <nav class="hidden lg:flex items-center space-x-6">
                @foreach (['Home', 'About', 'Catalog'] as $menu)
                    ...
                    @endphp
                    <a href="{{ $url }}"
                        class="group relative inline-flex items-center gap-1 text-white font-medium transition duration-300 hover:text-[#f9e5c9]">
        ...
        <!-- RIGHT SIDE: SEARCH + CART + PROFILE + HAMBURGER -->
        <div class="flex items-center gap-4">
            ...
            <div x-data="{ focus: {{ request('q') ? 'true' : 'false' }}, query: @js(request('q')) }" class="relative text-white">
                <input
                    ...
                    class="bg-white/10 sm:bg-white/20 text-white placeholder-white/70 backdrop-blur-sm rounded-full pl-10 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#f9e5c9] focus:bg-white/30 transition-all duration-300"
                    :class="focus ? 'w-48 sm:w-60' : 'w-32 sm:w-40 sm:hover:w-60'">
            ...
            <div class="hidden lg:flex items-center gap-4">
                ...
                <div class="relative inline-block text-left">
                    <button id="profileDropdownButton" type="button"
                        class="inline-flex justify-center items-center w-8 h-8 rounded-full ring-2 ring-[#b49875] hover:ring-[#f9e5c9] hover:scale-110 shadow-md transition duration-300"
                        ...
                        <img
                            ...
                            alt="Profile Picture" loading="lazy" class="w-8 h-8 rounded-full object-cover">
                        @else
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="w-6 h-6 text-white group-hover:text-[#f9e5c9]" fill="currentColor"
                            viewBox="0 0 24 24">
                ...
                @else
                <a href="{{ route('login') }}"
                    class="px-5 py-2 text-sm font-semibold border border-white text-white rounded-full hover:bg-white hover:text-[#b49875] transition duration-300 shadow-md">
                    Login →
                </a>
                @endauth
            </div>
    ...
    <div x-show="open" x-cloak
        ...
        class="lg:hidden bg-[#b49875]/60 sm:bg-[#b49875]/70 sm:backdrop-blur-md border border-[#e4d2b8]/30 rounded-2xl p-5 text-white shadow-md space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4 text-[15px] font-medium tracking-wide">
            @foreach (['Home', 'About', 'Catalog'] as $menu)
                ...
                @endphp
                <a href="{{ $url }}"
                    class="block text-center py-3 rounded-xl bg-white/10 hover:bg-[#f9e5c9]/90 hover:text-[#9c7b59] ring-1 ring-white/10 transition-all duration-300 backdrop-blur-sm">
                    {{ $menu }}
                </a>
            @endforeach
            ...
            <!-- Cart -->
            <a href="/cart"
                class="flex items-center justify-center gap-2 py-3 rounded-xl bg-white/10 hover:bg-[#f9e5c9]/90 hover:text-[#9c7b59] ring-1 ring-white/10 transition-all duration-300 backdrop-blur-sm">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none"
                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M3 3h2l.4 2M7 13h10l4-8H6.4M7 13L5.4 5M7 13l-2 9m14-9l2 9m-6-4a1 1 0 11-2 0 1 1 0 012 0zm-6 0a1 1 0 11-2 0 1 1 0 012 0z" />
                </svg>
                <span class="text-sm">Cart ({{ $cartCount ?? 0 }})</span>
            </a>
        </div>
        ...
        <div class="grid gap-3">
            @auth
                <a href="/edit"
                    class="block py-3 text-center rounded-xl bg-white/10 hover:bg-[#f9e5c9]/90 hover:text-[#9c7b59] ring-1 ring-white/10 transition duration-300 backdrop-blur-sm">
                    Edit Profile
                </a>
                <a href="#" onclick="event.preventDefault(); document.getElementById('logout-form-mobile').submit();"
                    class="block py-3 text-center rounded-xl bg-[#b14c3c]/90 hover:bg-[#e55b4d] text-white font-semibold shadow transition duration-300">
                    Logout
                </a>
                <form id="logout-form-mobile" method="POST" action="{{ route('logout') }}" class="hidden">@csrf</form>
            @else
                <a href="{{ route('login') }}"
                    class="block py-3 text-center font-semibold rounded-xl bg-white/10 hover:bg-[#f9e5c9]/90 hover:text-[#9c7b59] ring-1 ring-white/10 transition duration-300 backdrop-blur-sm">
                    Login →
                </a>
            @endauth
        </div>
